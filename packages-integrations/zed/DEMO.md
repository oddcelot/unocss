...
This branch (`demo/zed-language-server`) is set up so anyone can check out the repo
and see the language server (and the [`unocss/zed`](https://github.com/unocss/zed)
extension that wraps it) working against the bundled `examples/`.
...
2. Clone [`unocss/zed`](https://github.com/unocss/zed), then in Zed: command
   palette → **`zed: install dev extension`** → select that clone's directory.
...
utilities still highlight (added on the `feat/zed-language-server` branch).
